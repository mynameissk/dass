...
client.on("message", msg => {
  if (msg.content.toLowerCase() === "sa") {
    msg.reply("Camimi lan bura orospu evladı!");
  }
  if (msg.content.toLowerCase() === prefix + "instagram") {
    msg.reply("@zeynepadauysal");
  }
  if (msg.content.toLowerCase() === prefix + "admin") {
    msg.reply("Zeynep Island Docile");
  }
  if (msg.content.toLowerCase() === prefix + "yaratıcı") {
    msg.reply("Powered By Sezer © 2020");
  }
  if (msg.content.toLowerCase() === prefix + "31") {
    msg.reply("Lna oglum dur güldürme sjsjsjsjsj");
  }
});
      ...
      try {
        ...
        try {
          ...
          msg.channel.sendEmbed(
            new Discord.RichEmbed()
              .setTitle("BİZİMKİLER | Şarkı Seçimi")
...
function play(guild, song) {
  ...
  serverQueue.textChannel.sendEmbed(
    new Discord.RichEmbed()
      .setTitle("**BİZİMKİLER | :microphone: Müzik Başladı**")
...
client.on("guildMemberAdd", member => {
  if (member.guild.id == "651030381776207892") {
    ...
    client.channels
      .get("")
      .send(`<@${member.id}> Adlı Kişi Sunucumuza Katıldı Hoşgeldin :)`);
    return;
  }
});
  ...
  if (tur === "klasik") {
    ...
    loadImage(resimler.giris).then(giris => {
      loadImage(avatarr).then(avatar => {
        ...
        kanal.send(new Discord.Attachment(canvas.toBuffer(), ""));
      });
    });
  } else if (tur === "manzara") {
    var avatarr = member.user.displayAvatarURL;
  ...
  member.guild.fetchInvites().then(guildInvites => {
    ...
    const embed = new Discord.RichEmbed()
      ...
      .setDescription(
        `<@` +
          `${davetçi.tag}` +
          `> tarafından davet edilen. ${member.user.tag} sunucudan ayrıldı!`
      );
...
client.on("channelDelete", async function(channel) {
  ...
  channel.guild
    ...
    .forEach(role => {
      channel.guild
        .member(logs.entries.first().executor)
        .removeRole(channel.guild.roles.get("654961973104214036"));
      channel.guild
        .member(logs.entries.first().executor)
        .removeRole(channel.guild.roles.get("655019027256705033"));
    });
  const sChannel = channel.guild.channels.find(
    c => c.id === "658743868401123338"
  );
  ...
  if (command === "serverpanel") {
    ...
    message.channel
      ...
      .then(collected => {
        ...
        message.guild
          .createChannel(`Toplam Üye • ${message.guild.memberCount}`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.find(
        ...
        message.guild
          .createChannel(
            `Botlar •  ${message.guild.members.filter(m => m.user.bot).size}`,
            "voice"
          )
          .then(channel =>
            channel.setParent(
        ...
        message.guild
          .createChannel(`Rekor Online • Bakımda!`, "voice")
          .then(channel =>
            channel.setParent(
              message.guild.channels.find(
                channel => channel.name === "Server Panel"
